...
class CRUDWGInterface(CRUDBase[WGInterface, WGInterfaceCreate, WGInterfaceUpdate]):
    ...
    def create_wg_quick_config_file(self, orm_server: WGInterface):
        logger.info(
            f"Writing database server config file to: {pathlib.Path(settings.wg_if_file_path).resolve()}"
        )
        result = []
        ...
        filename = settings.wg_if_file_path
        os.makedirs(os.path.dirname(filename), exist_ok=True)
        with open(file=settings.wg_if_file_path, mode="w", encoding="utf-8") as f:
            f.write(os.linesep.join(result))
        logger.debug(f"Server Config saved to -->{settings.wg_if_file_path}")
